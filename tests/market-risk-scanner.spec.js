...
test("candidate simulation tab renders the latest snapshot", async ({ page }) => {
  ...
  await expect(page.locator("#simInvested")).toHaveText(/%/);
  ...
  await expect(page.locator("#simCumInvested2")).toHaveText(/\$\d/);
  await expect(page.locator("#simPnlChart")).toBeVisible();
  await expect(page.locator("#panel-simulation table").first().getByRole("columnheader", { name: "累計投入" })).toBeVisible();
